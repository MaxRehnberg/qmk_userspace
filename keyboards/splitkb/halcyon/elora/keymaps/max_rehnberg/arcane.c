...
static bool handle_arcane_release(keyrecord_t* record) {
    ...
    if (arcane_repeating) {
        ...
        arcane_repeating = false;
    }

    return true;
}

void process_right_arcane(keyrecord_t* record) { // RARCANE definitions
    if (handle_arcane_release(record)) {
        return;
    }

    arcane_repeating = false;

    const uint16_t keycode = get_last_keycode();
    ...
    switch (keycode) {
        ...
        default:
            arcane_repeating = true;
            repeat_key_invoke(&record->event);
            break;
    }
}

void process_left_arcane(keyrecord_t* record) { // LARCANE definitions
    if (handle_arcane_release(record)) {
        return;
    }
